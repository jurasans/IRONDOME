...
public class DroneStriker {
    public static final int DUMBFIRE_FLIGHTPLAN = 0;// Straight forward
    public static final int EVASOR_FLIGHTPLAN = 1;// tries to evade incoming interception misses
    ...
    public static final int MIRAGE_WARHEAD = 4;// splits into 2-4 HEAT misses
    public static final int HEAT_WARHEAD = 5;// slightly faster kills building
    public static final int KORNET_WARHEAD = 6;// kills every building in flight path
     ...
     * 
     * @param id
     *            the defence system currently deployed
     */
    ...
    /**
     * Make a projectile : target random city in a country.<br>
     * Initialize random starting location. initilize firing solution.<br>
     * 
     * @param speed
     *            speed of projectile
     * @param typeOfMissle
    ...
    DroneStriker(int speed, int typeOfMissle, int typeOfWarhead) {
        // Basic attribute assignment speed,target,starting points and weapon types
        this.speed = speed;
        ...
        if (typeOfMissle == DroneStriker.SINUSOIDAL_FLIGHTPLAN) {
            // set right above the mark
            setLocator(new Point(target.x, locator.y));
            misslesPath = new ArrayList<Point>();
            flightPath = new Line2D.Double(locator.x, locator.y, target.getCenterX(), target.getCenterY());
        ...
        if (typeOfMissle == DroneStriker.EVASOR_FLIGHTPLAN) {
            ...
            evasorLife = (byte) 20;
            misslesPath = new ArrayList<Point>();
            // create a radar to follow rocket around
            evasorRadar = new Ellipse2D.Double(locator.x - 250, locator.y - 250, 500, 500);
            flightPath = new Line2D.Double(locator.x, locator.y, target.getCenterX(), target.getCenterY());
     ...
     *            end y
     * @param points
     *            the array to fill up
     * @param interval
     *            keep 0 if u don't want any trouble. used for another developing algorithm
     * @return
     */
     ...
     *            start y
     * @param repeat
     *            desired length of operaton
     * @param points
     *            for the firing solution projectile path .the array to be written to
     * @return
     */
    void sineWaver(int x1, int y1, double repeat, ArrayList<Point> points) {
        // x location where sin wave is being generated on x
        int xBase = y1;
        // phase
        int top = x1;
        // magnitude of sin wave
        int yScale = 100;
    ...
    // old circle like algorithm is in code dump
    ...
    public void homing() {
        ...
        if (!isDestroyed()) {
            ...
            if (getType() == DroneStriker.DUMBFIRE_FLIGHTPLAN) {
                ...
                // interceptors only care about the explosion.
                if (!isInterceptor()) {
                ...
                if ((!isInterceptor())
                        && (getMissles().get(0).distance(new Point((int) target.getCenterX(), (int) target.getCenterY())) < 10)) { // strike
                    IsDestroyed(true);
                ...
                if ((isInterceptor()) && (getMissles().get(0).distance(getFlightPath().getP2()) < 10)) { // case of missle being interceptor
                    // missle
                    ...
                    // blast render
                    load.sm.getGraphics().fillOval((int) getFlightPath().getP2().getX() - 25, (int) getFlightPath().getP2().getY() - 25,
                            50, 50);
                    IsDestroyed(true);
                    // TODO add behavior for the case of the interceptor meeting another object from flyer collection
                }
            }
            // case for sinusoidal flightpath stage 1
            if (getType() == DroneStriker.SINUSOIDAL_FLIGHTPLAN) {
                ...
                if (misslesPath.get(0).distance(new Point((int) target.getCenterX(), (int) target.getCenterY())) < 150) { // second stage of
                    // sinusoidal
                    // flight path
            ...
            if (getType() == DroneStriker.EVASOR_FLIGHTPLAN) {
                ...
                for (DroneStriker ds : flyers) {
                    ...
                    if ((!isInterceptor())
                            && (getMissles().get(0).distance(new Point((int) target.getCenterX(), (int) target.getCenterY())) < 10)) {
                        IsDestroyed(true);
                    }
